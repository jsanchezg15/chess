...
public class State {
	...
	public State getMinMax(int iteration, Thread th) {
		if(iteration == 1) {
			if(this.white_moves) {
			...
			if(this.white_moves) {
				...
				if(!next.isEmpty()) {
					State max = next.remove(0).getMinMax(iteration - 1, th);
					
					for(State state : next) {
						state = state.getMinMax(iteration - 1, max.getEquilibium(), th);
						if(max.getEquilibium() < state.getEquilibium())
							max = state;
					}
				...
				if(!next.isEmpty()) {
					State min = next.remove(0).getMinMax(iteration - 1, th);
					
					for(State state : next) {
						state = state.getMinMax(iteration - 1, min.getEquilibium(), th);
						if(min.getEquilibium() > state.getEquilibium())
							min = state;
					}
	...
	private State getMinMax(int iteration, double limit, Thread th) {
		if(iteration == 1) {
			if(this.white_moves) {
				Vector<State> next = next_states();
