...
public class State {
		...
		if(this.white_moves) {
			...
			for(int i = 0; i < 8; i++) {
				for(int j = 0; j < 8; j++) {
					...
					if(piece == 'O' || piece == 'P') {
						next_states.addAll(pawn_movement(i, j));
						System.out.println("i: " + (i + 1) + "  j: " + (j + 1) + "  Size: " + pawn_movement(i, j).size());
					}
					else if(piece == 'T' || piece == 'R')
						next_states.addAll(rook_movement(i, j));
					else if(piece == 'C')
	...
	public State getMinMaxSinPoda(int iteration) {
		if(iteration == 0) 
			return this;
		
		if(this.white_moves) {
			State max = next_states().get(0).getMinMaxSinPoda(iteration - 1);
			
			for(State state : this.next_states()) {
				state = state.getMinMaxSinPoda(iteration - 1);
				if(state.getEquilibium() > max.getEquilibium()) 
					max = state;
			}
			...
			State min = next_states().get(0).getMinMaxSinPoda(iteration - 1);
			
			for(State state : this.next_states()) {
				state = state.getMinMaxSinPoda(iteration - 1);
				if(state.getEquilibium() < min.getEquilibium()) 
					min = state;
			}
		...
		if(this.children.isEmpty())
			...
			if(this.white_moves) {
				...
				for(State state : this.children)
					next.add(state.getMinMaxSinPoda());
