...
public class State {
	...
	public double getEquilibium() { 
		if(this.equilibrium < 0) 
			this.equilibrium = table_calculate_equilibrium();
	...
	public double table_calculate_equilibrium() {
		// prev_new_calculate_equilibrium -> this
		// Puntúa +0.2 al contrario por si el rey está amenazado y te toca mover
		// Si el rey está amenazado y no te toca mover puntúa infinito o 0
		...
		for(int i = 0; i < 8; i++)
			for(int j = 0; j < 8; j++) {
				...
				if(piece < 95) {
					if(piece == 'O')  					// Peón 
						white_points = white_points + 1 + PieceValuator.PAWN_VALUE[i][j];
					else if(piece == 'C') 				// Caballo
						white_points = white_points + 3 + PieceValuator.KNIGHT_VALUE[i][j];
					else if(piece == 'A') 				// Alfil
						white_points = white_points + 3 + PieceValuator.BISHOP_VALUE[i][j];
					else if(piece == 'R') 				// Torre
						white_points = white_points + 5 + PieceValuator.ROOK_VALUE[i][j];
					else if(piece == 'T') 				// Torre que ya ha hecho un movimiento
						white_points = white_points + 5 + PieceValuator.ROOK_VALUE[i][j];
					else if(piece == 'Q')				// Reina
						white_points = white_points + 9 + PieceValuator.WHITE_QUEEN_VALUE[i][j];
					else if(piece == 'K' || piece == 'M') {
						white_king = true;
						white_points = white_points + PieceValuator.KING_VALUE[i][j];
					}
					else if(piece == 'P') 				// Peón capturable al paso
						white_points = white_points + 1 + PieceValuator.PAWN_VALUE[i][j];
				}
					...
					if(piece == 'o')
						black_points = black_points + 1 + PieceValuator.PAWN_VALUE[7 - i][j];
					else if(piece == 'c')
						black_points = black_points + 3 + PieceValuator.KNIGHT_VALUE[i][j];
					else if(piece == 'a')
						black_points = black_points + 3 + PieceValuator.BISHOP_VALUE[7 - i][j];
					else if(piece == 'r')
						black_points = black_points + 5 + PieceValuator.ROOK_VALUE[7 - i][j];
					else if(piece == 't')
						black_points = black_points + 5 + PieceValuator.ROOK_VALUE[7 - i][j];
					else if(piece == 'q')
						black_points = black_points + 9 + PieceValuator.BLACK_QUEEN_VALUE[i][j];
					else if(piece == 'k' || piece == 'm') {
						black_king = true;
						black_points = black_points + PieceValuator.KING_VALUE[7 - i][j];
					}
					else if(piece == 'p')
						black_points = black_points + 1 + PieceValuator.PAWN_VALUE[i][j];
				}
