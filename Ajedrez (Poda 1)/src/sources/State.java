...
public class State {
	...
	public State getMinMax(int num) {
		if(num == 0) {
			if(this.white_moves) {
			...
			State max = next.remove(0).getMinMax(num - 1);
			
			for(State state : next) {
				State my_state = state.getMinMax(num - 1, max.getEquilibium());
				if(max.getEquilibium() < my_state.getEquilibium())
					max = my_state; 
			}
			...
			State min = next.remove(0).getMinMax();
			
			for(State state : next) {
				State my_state = state.getMinMax(num - 1, min.getEquilibium());
				if(min.getEquilibium() > my_state.getEquilibium())
					min = my_state; 
			}
	...
	public State getMinMax(int num, double limit) {
		if(num == 0) {
			if(this.white_moves) {
				Vector<State> next = next_states();
			...
			for(State state : next_states()) {
				...
				State my_state;
				if(max == null)
					my_state = state.getMinMax(num - 1, 0);
				else
					my_state = state.getMinMax(num - 1, max.getEquilibium());
				
				if(my_state.getEquilibium() >= limit)
					return my_state;
				
				if(max == null)
					max = my_state;
				
				else if(max.getEquilibium() < my_state.getEquilibium())
					max = state; 
			}
			return max;
		}
			...
			for(State state : next_states()) {
				...
				State my_state;
				if(min == null)
					my_state = state.getMinMax(num - 1, Double.MAX_VALUE);
				else
					my_state = state.getMinMax(num - 1, min.getEquilibium());
				...
				else if(min.getEquilibium() > my_state.getEquilibium())
					min = state; 
			}
