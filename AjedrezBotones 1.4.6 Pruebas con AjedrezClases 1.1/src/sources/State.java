...
public class State {
	...
	public Vector<State> king_movement(int i, int j, boolean kingside) {
		...
		// HAY QUE BORRAR ESTA LÍNEAAAAAAAAAAAA
		// HAY QUE BORRAR ESTA LÍNEAAAAAAAAAAAA
		// HAY QUE BORRAR ESTA LÍNEAAAAAAAAAAAA
		kingside = false;
		// HAY QUE BORRAR ESTA LÍNEAAAAAAAAAAAA
	...
	public State getMinMaxSinPoda(int iteration) {
		return getMinMaxSinPoda(iteration, new MiBoolean(false));
	}
	
	public State getMinMaxSinPoda(int iteration, MiBoolean stop) {
		if(stop.value)
			return null;
		...
		if(iteration == 1) {
			...
			if(this.white_moves) {
				...
				if(!next.isEmpty()) {
					...
					for(State state : next) {
						state = state.getMinMax(iteration - 1, stop);
						if(max.getEquilibium() < state.getEquilibium())
							max = state;
					}
