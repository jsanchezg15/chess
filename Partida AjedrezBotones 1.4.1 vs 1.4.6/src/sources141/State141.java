...
public class State141 {

	private char[][] board;
	private double equilibrium;
	protected boolean white_moves;
	private State141 parent;
	private Vector<State141> children;
	
	public State141(char[][] board, boolean white_moves) {
		this.board = board;
		this.equilibrium = -1;
		this.white_moves = white_moves;
		this.parent = null;
		this.children = new Vector<State141>();
		
		set_pawns();
	}
	
	
	public State141 getParent() {return this.parent;}
	
	public void setWhiteMoves(boolean wm) { this.white_moves = wm; }
	
	public void setParent(State141 parent) { this.parent = parent; }
	...
	public double getEquilibium() { 
		if(this.equilibrium < 0) 
			this.equilibrium = calculate_equilibrium_king_menace();
	...
	public double calculate_equilibrium() {
		double white_points = 0.5 + Math.random() / 20;
		double black_points = 0.5 + Math.random() / 20;
		boolean white_king = false;
		boolean black_king = false;
	...
	public Vector<State141> next_states() {
			
		if(!this.children.isEmpty())
			return this.children;
		
		Vector<State141> next_states = new Vector<State141>();
		
		if(this.getEquilibium() == 0) {
			State141 state = new State141( State141.black_wins(), !this.white_moves );
			state.setParent(this);
			next_states.add(state);
			
		}
		else if(this.getEquilibium() == Double.MAX_VALUE) {
			State141 state = new State141( State141.white_wins(), !this.white_moves );
			state.setParent(this);
			next_states.add(state);
		}
	...
	public Vector<State141> king_movement(int i, int j, boolean kingside) {
		Vector<State141> new_states = new Vector<State141>();
		char[][] new_state; 
			...
			if(kingside) {
				if(board[7][0] == 'R' && board[7][1] == '_' && board[7][2] == '_' && board[7][3] == '_') {
					if(  !(is_menaged(7, 4, true) || is_menaged(7, 3, true) || is_menaged(7, 2, true)  )  ) {
						...
						new_states.add(new State141(new_state, false));
					}
				}
			...
			if(kingside) {
				if(board[0][0] == 'r' && board[0][1] == '_' && board[0][2] == '_' && board[0][3] == '_') {
					if(  !(is_menaged(0, 4, false) || is_menaged(0, 3, false) || is_menaged(0, 2, false)  )  ) {
						...
						new_states.add(new State141(new_state, true));
					}
				}
		...
		for(State141 state : new_states) 
			state.setParent(this);
		
		return new_states;	
	}

	public Vector<State141> bishop_movement(int i, int j) {
		Vector<State141> new_states = new Vector<State141>();
		char[][] new_state; 
	...
	public Vector<State141> knight_movement(int i, int j) {
		Vector<State141> new_states = new Vector<State141>();
		char[][] new_state; 
	...
	public Vector<State141> rook_movement(int i, int j) {
		Vector<State141> new_states = new Vector<State141>();
		char[][] new_state; 
	...
	public Vector<State141> queen_movement(int i, int j) {
		Vector<State141> new_states = new Vector<State141>();
		char[][] new_state; 
	...
	public Vector<State141> pawn_movement(int i, int j) {
		Vector<State141> new_states = new Vector<State141>();
		char[][] new_state; 
	...
	public State141 getMinMax(int iteration, MiBoolean stop) {
		if(stop.value)
			return null;
		
		if(iteration == 1) {
			if(this.white_moves) {
				Vector<State141> next = next_states();
				
				if(!next.isEmpty()) {
					State141 max = next.remove(0);
					
					for(State141 state : next) 
			...
			if(this.white_moves) {
				...
				if(!next.isEmpty()) {
					State141 max = next.remove(0).getMinMax(iteration - 1, stop);
				...
				if(!next.isEmpty()) {
					State141 min = next.remove(0).getMinMax(iteration - 1, stop);
		...
		return State141.void_state();
	}

	private State141 getMinMax(int iteration, double limit, MiBoolean stop) {
		if(stop.value)
			return null;
	...
	public static State141 void_state() {
		...
		return new State141(new_table, true);
	}
	...
	public State141 move(int i1, int j1, int i2, int j2) {
		...
		State141 move = new State141(clone, !this.white_moves);
	...
	public boolean board_equals(State141 state) {
		for(int i = 0; i < 8; i++)
			for(int j = 0; j < 8; j++) 
	...
	public boolean exist_move(State141 state) {
		for(State141 st : this.next_states()) 
